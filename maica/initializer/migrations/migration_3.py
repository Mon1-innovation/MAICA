...
async def migrate():
    ...
    try:
        async with DatabaseUtils.engine_auth.begin() as conn:
            ...
            if not exist:
                sync_messenger(info='[migration-3] Column does not exist, creating...', type=MsgType.DEBUG)
                await conn.execute(text('ALTER TABLE users ADD COLUMN suspended_until DATETIME DEFAULT NULL'))
            else:
                sync_messenger(info='[migration-3]Column already exists', type=MsgType.DEBUG)
