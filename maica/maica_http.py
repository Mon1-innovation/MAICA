        ...
        if verification_result:
            if len(str(content)) < 100000:
                try:
                    ...
                    result = await hduplex_instance.send_query(sql_expression_1, (hduplex_instance.options['vfc']['user_id'], chat_session))
                    if result:
                        pers_id = result[0]
                        sql_expression_2 = 'UPDATE persistents SET content = %s WHERE persistent_id = %s'
                        await hduplex_instance.send_modify(sql_expression_2, (content_text, pers_id))
                    else:
                        sql_expression_2 = 'INSERT INTO persistents (user_id, chat_session_num, content) VALUES (%s, %s, %s)'
                        await hduplex_instance.send_modify(sql_expression_2, (hduplex_instance.options['vfc']['user_id'], chat_session, content_text))
                except Exception:
            ...
            if len(str(content)) < 100000:
                try:
                    ...
                    if result:
                        ...
                        await hduplex_instance.send_modify(sql_expression_2, (content_text, trgs_id))
                    else:
