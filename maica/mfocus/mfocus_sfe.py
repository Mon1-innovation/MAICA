...
class SfBoundCoroutine():
    def __init__(self, fsc: FullSocketsContainer, maica_pool: DbPoolCoroutine) -> None:
    ...
    async def _find_session_or_default(self) -> int:
        sql_expression = 'SELECT persistent_id FROM persistents WHERE user_id = %s AND chat_session_num = %s'
    ...
    async def _check_expired_or_not(self) -> bool:
        if self.p_id and self.timestamp and self.sf_content:
            sql_expression_1 = 'SELECT timestamp FROM persistents WHERE persistent_id = %s'
    ...
    async def reset(self, force=False) -> None:
        if await self._check_expired_or_not() or force:
            try:
                sql_expression = 'SELECT persistent_id, content, timestamp FROM persistents WHERE user_id = %s AND chat_session_num = %s'
                ...
                self.p_id = self.timestamp = None
                self.sf_content = {}
                await common_context_handler(self.websocket, 'mfocus_no_persistent', 'No persistent found for MFocus, using empty', '404', traceray_id=self.traceray_id)
            self.sf_forming_buffer = {}
            self.sf_forming_buffer.update(self.sf_content)
        else:
    ...
    async def mfocus_find_info(self, query):
        try:
            information = (await wrap_run_in_exc(None, self.mfocus_form_info))
            system_init = """你是一个人工智能助手, 你接下来会收到一个问题和一系列信息.
注意不要对信息作任何改动. 你可以认为[player]是用户的名字, 莫妮卡是你的名字.
从信息中挑选1至5条最能用于回答的信息, 互不重复, 并以单行Python列表的形式输出. 如果你最终认为没有信息符合条件, 回答None.
...
Pick 1 to 5 independent items from the information that helps answering the question the most, and return them in a Python list. If you think no information provided is helpful at last, answer None.
Begin!"""
            ...
            await common_context_handler(self.websocket, 'mfocus_sfe_search', f"MFocus sfe searching persistent, response is:\n{response}\nEnd of MFocus sfe searching persistent", '201')
            ...
            if answer_post_think and not ReUtils.re_search_answer_none.search(answer_post_think):
                ...
                return success, None, ''
            return success, None, answer_fin
