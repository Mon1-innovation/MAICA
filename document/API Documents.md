...
因此, 用户应该使用论坛账号登录. 对于你自己的部署, 你可以接入自己的flarum论坛或用其它方式直接操作数据库.
...
其中, 若将reset设为true则先重置设置再应用传入设置, 否则在当前设置上update. 默认为false.

在chat_params中, 这三部分设置分别属于基础设置, 高级设置和超参数. 在v1.1更新之后, 其均作为chat_params传入.  
...
* mf_llm_concl: 要求agent模型生成最终指导, 并替代默认MFocus指导.
    ...
    - 启用时一般会无效化mf_constant_tools.

* mf_extraction_impl: 实验性功能, 从设定集(存档)中快速提取信息的方式, 并替代传统MFocus实现.
...
* mf_constant_pers: 实验性功能, 即使MFocus未调用信息提取, 也提供信息提取的结果.
    ...
    * 2: 仅预检索.
    ...
    - 如果mf_extraction_impl使用传统实现, 可能会慢到爆炸.

* mf_constant_tools: 即使MFocus未调用工具, 也提供一些工具的结果.
...
> 应当注意, 当seed为默认值null时, 生成时种子表现为模型的默认行为, 一般而言是随机种子.
...
MAICA会继续发送此前未发完的响应数据.

* 已生成完的部分将在极短时间内全部发出. 若重连时生成仍未完成, 后续的流式输出会继续.
* 断点续传保护功能介入后, 已中断的连接协程会继续运行, 直到全流程完成. 在此期间重连会被视为重复登录, 其行为受后端配置MAICA_KICK_STALE_CONNS影响.
* 所有内容续传完后, 后端会发送status为maica_reconn_buffer_drained的提示. 若不存在待传内容, 则会发送maica_reconn_buffer_empty.
* 下一次断点续传保护开始记录时, 上一次的记录内容就会被销毁.
* 流式传输, 非流式传输, MTrigger内容等都支持断点续传保护功能.
...
`{"type": "query", "chat_session": "1", "query": "你好啊", "pprt": 实时后处理配置}`

* 这一功能是前端可以实现的, 但为方便开发和简化流程起见, 于后端提供支持.
* 自v1.2.006.rc1后, 所有形式的核心输出都会由pprt处理, 且不再可禁用.
...
* yield_interval: 每收到指定多个chunk尝试一次断句, 若断句引擎认为不应断句则尝试下一个数字. 成功断句时重置计数, 耗尽后最终变为1. 合理配置以兼顾流畅和表现.
  > 仅当自动断句启用时生效.
  
* split_limit: 期望的每句长度(编码计算, 每个中文字符占3, 每个英文字母占1), 断句引擎会尽可能提供接近该长度的合理断句. 注意这不是一个硬限制, 最多可能超出20字符左右.
...
#### 应当注意, 为拓展性起见, MAICA-MTTS作为MAICA后端的可选依赖安装, 并不直接属于MAICA仓库. 对于关于MAICA-MTTS的问题, 参见其附带的文档.
...
* 自v1.2.000.rc11后, sping可以在登录前发送.
* 说实话我不知道这个能有什么用, 但是先做了吧.
* 如果单纯用于连接保活, 你也许应该用这个.
...
> 自v1.2更新后, 短连接响应会在失败时携带失败状态码, 若未指定则为400.   
> 短连接输出形式原则上保持一致. 若响应形式不正确, 请检查端点, 中间件等.
