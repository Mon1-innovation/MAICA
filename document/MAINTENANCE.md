...
视觉 URL 仍由模型服务实际抓取。`MAICA_VISION_HOST_ALLOWLIST` 支持主机名、IP、CIDR、`!` 拒绝规则及 `!*` 默认拒绝；主机规则校验与模型服务实际抓取之间仍存在 DNS 变化窗口，因此还应在网络层阻断模型容器访问云元数据、数据库和管理网段。预编译 SERP 二进制属于独立外部组件，升级时应核对其来源和校验值。
