...
# }
    ...
    messages.extend([{'role': 'user', 'content': input}, {'role': 'assistant', 'content': output}, {'role': 'user', 'content': '观察以上对话历史记录, 依据你上一次作出的回应思考并调用工具, 每个工具最多只能调用一次. 不要在未经用户明确指示的情况下调用除好感以外的工具. 你的选择必须与你上一次作出的回应一致. 你应在回答前加上\'Final Answer:\'字样.'}])
